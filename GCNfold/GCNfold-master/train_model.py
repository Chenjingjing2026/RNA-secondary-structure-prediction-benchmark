...
# base_dir = '/home/chenjingjing/Models/GCNfold/GCNfold-master'
# # load config
# args = get_args()
...
# print(trainset)
#
# # seq_len = trainset[1].shape[1]  # 假设 trainset[1] 是序列数据
# seq_len = trainset.seq.shape[1]  # (29, 600, 4)
# # net_params['in_dim'] = dataset.train[0].ndata['feat'].shape[1]  # 获取节点特征的维度
...
def evaluate_test(model, device, val_loader, epoch):
    ...
    with torch.no_grad():
        ...
        print("exact_f1:", type(exact_f1), exact_f1)
        print("exact_p:", type(exact_p), exact_p)
        print("exact_r:", type(exact_r), exact_r)
        print("exact_mcc:", type(exact_mcc), exact_mcc)
...
def all_data_test(model, device, val_loader, epoch):
    ...
    with torch.no_grad():
        for iter, (batch_graphs, contacts, seq_embeddings, matrix_reps, seq_lens) in enumerate(val_loader): 
            ...
            map_no_train = (u_no_train > 0.5).float()
            # 在循环内添加调试代码
            print("真实标签中1的数量:", torch.sum(contacts_batch > 0).item())
            print("预测值的范围:", torch.min(u_no_train).item(), torch.max(u_no_train).item())
            result_exact = list(map(lambda i: evaluate_exact(map_no_train.cpu()[i], contacts_batch.cpu()[i]), range(contacts_batch.shape[0])))
            result_shift = list(map(lambda i: evaluate_shifted(map_no_train.cpu()[i], contacts_batch.cpu()[i]), range(contacts_batch.shape[0])))
            result_auc = list(map(lambda i: calculate_auc(map_no_train.cpu()[i], contacts_batch.cpu()[i]), range(contacts_batch.shape[0])))
...
def train(model, dataset_name, optimizer, device, data_loader, epoch):
    ...
    evaluate_epi = 0
    itertotal = 0
    print('len(data_loader)',data_loader.__len__())
    print('len(data_loader)',len(data_loader))  # 查看data_loader是否有数据
    # print('train_loader.dataset',train_loader.dataset)  # 打印数据集内容
    for iter1, (rnagraphs, contacts, seq_embeddings, matrix_reps, seq_lens) in enumerate(data_loader):
    # for iter1, (rnagraphs, contacts, seq_embeddings, matrix_reps, seq_lens) in enumerate(train_loader):
        itertotal = iter1
...
def test_only(model, device, test_loader):
    """仅测试模式，不进行训练"""
    print("\n进入仅测试模式...")
    test_auc, test_f1, test_p, test_r, test_mcc, test_f1_shift, test_p_shift, test_r_shift = all_data_test(model,
                                                                                                           device,
                                                                                                           test_loader,
                                                                                                           0)

    print('\n最终测试结果:')
    print('AUC(曲线下面积): {:.4f}'.format(test_auc))
    print('F1分数: {:.4f}, 精确率: {:.4f}, 召回率: {:.4f}, MCC(马修斯相关系数): {:.4f}'.format(test_f1, test_p, test_r,
                                                                                               test_mcc))
    print('位移F1: {:.4f}, 位移精确率: {:.4f}, 位移召回率: {:.4f}'.format(test_f1_shift, test_p_shift, test_r_shift))
    ...
    return {
        ...
        'recall': test_r,
        'mcc': test_mcc,
        'f1_shift': test_f1_shift,
        'precision_shift': test_p_shift,
        'recall_shift': test_r_shift
    }
...
if __name__ == '__main__':
    ...
    drop_last = True
    train_loader = data.DataLoader(trainset, batch_size=BATCH_SIZE, shuffle=True, drop_last=drop_last,
                                   collate_fn=dataset.collate)
    val_loader = data.DataLoader(valset, batch_size=BATCH_SIZE, shuffle=False, drop_last=False,
                                 collate_fn=dataset.collate)
    test_loader = data.DataLoader(testset, batch_size=BATCH_SIZE, shuffle=False, drop_last=False,
                                  collate_fn=dataset.collate)
    print('Data Loading Done!!!')
    ...
    if only_test:
        # 仅测试模式
        test_results = test_only(contact_net, device, test_loader)
    else:
        # define optimizer and loss function
        gcn_optimizer = optim.Adam(contact_net.parameters())
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(gcn_optimizer, mode='min', factor=config.lr_reduce_factor,
        ...
        for epoch in range(epochs):
            ...
            print('epoch: {}, time: {:.2f}, lr: {}'.format(epoch, time.time() - start,
                                                           gcn_optimizer.param_groups[0]['lr']))
            print('epoch: {}, train_loss: {:.4f}'.format(epoch, epoch_train_loss))

            #
            if (epoch + 1) % val_epi == 0:
                print('epoch: {}, val_loss: {:.4f}, auc: {:.4f}, f1: {:.4f}, p: {:.4f}, r: {:.4f}, mcc: {:.4f}'.format(
                    epoch, epoch_val_loss,
                    model_val_auc, val_f1,
                    val_p, val_r, val_mcc))
                print('epoch: {}, f1_shift: {:.3f}, p_shift: {:.4f}, r_shift: {:.4f} \n'.format(epoch, val_f1_shift,
                                                                                                val_p_shift,
                                                                                                val_r_shift))
            else:
                print('epoch: {}, val_loss: {:.4f}, auc: {:.4f} \n'.format(epoch, epoch_val_loss, model_val_auc))
        ...
        all_data = {'Epoch': list(epoch), 'Train Loss': epoch_train_losses, 'Val Loss': epoch_val_losses,
                    'Val AUC': auc_val_lists}
        save_to_csv(data=all_data, file_name='GCNFold Exper', save_format='csv', save_type='col')

        # test all data
        test_auc, test_f1, test_p, test_r, test_mcc, test_f1_shift, test_p_shift, test_r_shift = all_data_test(
            contact_net, device, test_loader, epoch)
        print(
            'test results, auc: {:.4f}, f1: {:.4f}, p: {:.4f}, r: {:.4f}, mcc: {:.4f}'.format(test_auc, test_f1, test_p,
                                                                                              test_r, test_mcc))
        print('f1_shift: {:.4f}, p_shift: {:.4f}, r_shift: {:.4f}'.format(test_f1_shift, test_p_shift, test_r_shift))
...
# print('test results, auc: {:.4f}, f1: {:.4f}, p: {:.4f}, r: {:.4f}, mcc: {:.4f}'.format(test_auc, test_f1, test_p, test_r, test_mcc))
# print('f1_shift: {:.4f}, p_shift: {:.4f}, r_shift: {:.4f}'.format(test_f1_shift, test_p_shift, test_r_shift))
#
