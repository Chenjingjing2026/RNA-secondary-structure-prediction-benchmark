...
from data.RNAGraph import RNAGraphDatasetDGLTest

dataset = 'bprna_new'
# data_dir = os.getcwd()  # /content/drive/MyDrive/GCNfold
data_dir= '/home/chenjingjing/Models/GCNfold/GCNfold-master'
# datapath = os.path.join(data_dir, '/home/chenjingjing/DATA/rivas/TrainSetA/short/bpseq')
datapath= '/home/chenjingjing/DATA/data/bprna-new/without_pes/short/bpseq'
seed = 0

# select all the 5s files
file_list = os.listdir(datapath)
# file_list = list(filter(lambda x: x.endswith(".ct"), file_list))
file_list = list(filter(lambda x: x.endswith(".bpseq"), file_list))
...
# dglgraph
rna_datset = RNAGraphDatasetDGLTest(data_dir, seq_list, seq_encoding_list_padded, stru_encoding_list_padded, 
                                    seq_len_list_600, file_list_600, pairs_list)

savepath = os.path.join(data_dir, 'mydata', dataset)
parent_dir = os.path.dirname(savepath)
...
with open(savepath + '/test.pkl', 'wb') as f:
    pickle.dump(rna_datset.data, f)
