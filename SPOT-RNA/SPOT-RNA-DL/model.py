...
class ResBlock(nn.Module):
    ...
    def __init__(self, dim=48, dilation=1):
        ...
        self.conv1 = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=3, padding=1 + (dilation - 1),
                               dilation=dilation)
        self.conv2 = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=5, padding=2 + (dilation - 1),
                               dilation=dilation)
...
class Transformer(nn.Module):
    ...
    Transformer Encoder
    """
    def __init__(self, num_head=8, d_model=128, num_layer=6):
        super(Transformer, self).__init__()
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=num_head, dim_feedforward=d_model*4)
