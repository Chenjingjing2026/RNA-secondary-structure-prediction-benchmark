...
if DATASET=='shuffle_long':
    ...
    val_file = 'valid.txt'
    test_file = 'test.txt'
elif DATASET=='Crossfamily':
    train_file ='train.txt'
    val_file = 'valid.txt'
    test_file = 'test.txt'

# 在保存模型之前添加
model_dir = os.path.dirname(args.sessName)
...
if Train:
    ...
    for epoch in range(args.epoch):
        ...
        i = 0
        for (rna_name, seq_matrix), dot_matrix in tqdm(training_loader, 
                                                desc=f"Epoch {epoch}/{args.epoch}"):
            try:
                ...
                epoch_train_preci.append(preci.item())
                epoch_train_recall.append(recall.item())
                epoch_train_MCC.append(mcc.item())
    
                del loss, pred, seq_matrix, dot_matrix
                t.cuda.empty_cache()
            except Exception as e:
                print(f'Error:{rna_name}, {len(seq_matrix)}, {e}')
...
test_loader = SPOTDataSetDot(os.path.join(args.datadir, "test_medium"), device=device, flist=test_file)
print('Loaded Test Data success')
print('Start Evaluate')
# test_acc, test_loss, test_preci, test_recall, test_mcc = evaluate_model1(model, test_loader)
test_F1, test_loss, test_preci, test_recall, test_mcc = test_evaluate_model(model, test_loader)
# test_F1 = 2 * (test_preci * test_recall) / (test_preci + test_recall)
print("test_loss=%.5f, test_preci=%.5f, test_recall=%.5f, test_F1=%.5f, test_MCC=%.5f" % (
    test_loss, test_preci, test_recall, test_F1, test_mcc))
