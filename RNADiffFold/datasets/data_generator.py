...
class ParserData(object):
    def __init__(self, path):
        ...
        self.seq_max_len = max([x[2] for x in self.data])
    ...
    def get_item(self,idx):
        ...
        if idx < 0 or idx >= self.len:
            ...
            return self.get_item(np.random.randint(0, self.len))
        item=self.data[idx]            
        data_dict={}
        # get_data_fcn()
        
        data_dict['data_fcn_2']=torch.tensor(item[0]).float()
        data_dict['data_seq_raw']=item[1]  # 似乎不用变
        data_dict['data_length']=torch.tensor(item[2])
        data_dict['data_name']=item[3]
        data_dict['contact']=torch.tensor(pairs2map(item[4], self.set_max_len)).long()   #为啥要unsqueeze(1) ?
        data_dict['data_seq_encode_pad']=torch.tensor(padding(seq_encoding(item[1]), self.set_max_len))  # ????
        data_dict['set_max_len']=self.set_max_len
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        dset_idx = bisect.bisect_right(self.offset_list, index) - 1
...
def diff_collate_fn(batch, alphabet):  # 直接填充？不要分数据集？  这个应该被修改 TODO: 写信的coolate_fn按照这个batch内的最大长度来进行填充
    ...
    set_max_len = max([item['set_max_len'] for item in batch])  # 遵循源码意愿
    collated_batch={}
