...
ROOT_PATH = '/home/chenjingjing/Models/RNA_DiffFold/RNADiffFold/data'


def add_data_args(parser):
    # Data params
    parser.add_argument('--dataset', type=str, default='RNAStrAlign', choices=dataset_choices)
...
def get_data(args, alphabet):
    ...
    if args.dataset == 'RNAStrAlign':
        ...
        val = Dataset([join(ROOT_PATH, args.dataset, 'valid')])
        ...
        train = Dataset([join(ROOT_PATH, args.dataset, 'test')], upsampling=True)
        val = Dataset([join(ROOT_PATH, args.dataset, 'test')])
        test = Dataset([join(ROOT_PATH, args.dataset, 'test')])
