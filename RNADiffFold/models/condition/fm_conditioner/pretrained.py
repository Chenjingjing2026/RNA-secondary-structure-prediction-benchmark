...
from .fm.model import RNABertModel
from .fm.data import Alphabet
import torch
...
def load_model_and_alphabet_local(model_location, theme="rna"):
    ...
    try:
        ...
        regression_data = torch.load(regression_location, map_location='gpu')
    except FileNotFoundError:
        regression_data = None
    return load_model_and_alphabet_core(model_data, regression_data, theme)


def load_model_and_alphabet_core(model_data, regression_data=None, theme="rna"):
    if regression_data is not None:
        model_data["model"].update(regression_data["model"])

    # alphabet = fm.Alphabet.from_architecture(model_data["args"].arch, theme=theme)
    alphabet = Alphabet.from_architecture(model_data["args"].arch, theme=theme)
