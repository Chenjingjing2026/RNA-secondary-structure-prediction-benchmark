...
def load_base_pairs_info():
    ...
    base_pairs_file = '/root/lanyun-tmp/RNAErnie/mydata/all_pk_pairs.txt'
...
def compare_bpseq_pk(ref, pred,header):
    ...
    # print('pred:',pred)
    # print('ref:',ref)
    file_id = header[0].split('/')[-1].replace('.bpseq','')
    ...
    for i in range(len(pred)):
        pred_val = pred[i]
        true_val = ref[i]
        is_target = (file_id in base_pair and i in base_pair[file_id])
        # 根据四种情况更新计数器
    ...
    return target_tp, target_fp, target_tn, target_fn,other_tp, other_fp, other_tn, other_fn



def compare_bpseq2(ref, pred,header):
    """compare bpseq reference and prediction
    ...
    other_tp, other_fp, other_tn, other_fn = 0, 0, 0, 0
    L = len(ref) - 1
    other_L = len(base_pair[file_id]) * 2
    target_L = L - other_L
    tp = fp = fn = 0
    ...
    target_tn = target_L * (target_L - 1) // 2 - target_tp - target_fp - target_fn
    other_tn = other_L * (other_L - 1) // 2 - other_tp - other_fp - other_fn
    return target_tp, target_fp, target_tn, target_fn,other_tp, other_fp, other_tn, other_fn
...
class SspTrainer(BaseTrainer):
    ...
    def eval(self, epoch, save_path=None):
        ...
        with tqdm(total=n_dataset,disable=self.args.disable_tqdm) as pbar:
            # init with default values
            target_res = defaultdict(list)
            ...
            for instance in self.eval_dataloader:
                ...
                if instance["input_ids"].shape[0] < 600:
                    ...
                    for header, seq, ref, sc, pred, bp in zip(headers, seqs, refs, scs, preds, bps):
                        ...
                        target_tp, target_fp, target_tn, target_fn,other_tp, other_fp, other_tn, other_fn = compare_bpseq_pk(ref,bp,headers)
                        target_x = (target_tp, target_fp, target_tn, target_fn)
                        other_x = (other_tp, other_fp, other_tn, other_fn )
                        
                        target_ret = self.compute_metrics(*target_x)
                        other_ret = self.compute_metrics(*other_x)
                        for k, v in target_ret.items():
                            target_res[k].append(v)
                        
                        for k, v in other_ret.items():
                            other_res[k].append(v)
                ...
                pbar.set_postfix(eval_fls='{:.3e}'.format(np.mean(target_res[self.name_pbar])))
                pbar.update(self.args.batch_size)

            # save best model
            target_metrics_dataset = {}
            other_metrics_dataset = {}
            for k, v in target_res.items():
                target_metrics_dataset[k] = np.mean(v)
            for k, v in other_res.items():
                other_metrics_dataset[k] = np.mean(v)
            ...
            target_tag_value = defaultdict(float)
            other_tag_value = defaultdict(float)
            
            # log results to screen/bash
            target_results = {}
            target_log = 'Test: Target\t' + self.args.task_name + "\t"

            
            # extract results
            for k, v in target_metrics_dataset.items():
                target_log += k + ": {" + k + ":.4f}\t"
                target_results[k] = v
                tag = "eval/" + k
                target_tag_value[tag] = v
            
            print(target_log.format(**target_results))

            # log results to screen/bash
            other_results = {}
            other_log = 'Test: other\t' + self.args.task_name + "\t"
            # log results to visualdl

            
            # extract results
            for k, v in other_metrics_dataset.items():
                other_log += k + ": {" + k + ":.4f}\t"
                other_results[k] = v
                tag = "eval/" + k
                other_tag_value[tag] = v
            
            print(other_log.format(**other_results))
