...
class SspTrainer(BaseTrainer):
    ...
    def eval(self, epoch, save_path=None):
        ...
        with tqdm(total=n_dataset,disable=self.args.disable_tqdm) as pbar:
            ...
            for instance in self.eval_dataloader:
                ...
                if instance["input_ids"].shape[0] < 600:
                    ...
                    for header, seq, ref, sc, pred, bp in zip(headers, seqs, refs, scs, preds, bps):
                        ...
                        # 保存详细结果到列表
                        # 假设 ret 包含: precision, recall, f1s, mcc 等键
                        sample_detail = {
                            'name': header,
                            'precision': ret.get('precision', 0.0),
                            'recall': ret.get('recall', 0.0),
                            'f1': ret.get('f1s', 0.0),
                            'mcc': ret.get('mcc', 0.0)
                        }
            ...
            output_file_target = '/home/cjj/RNAErnie/output_ft/output_ft/bpRNA1m/bpRNA1m/bprna_new.txt'
            ...
            with open(output_file_target, 'w') as f:
                ...
                for result in detailed_results:
                    # 使用空格（" "）作为分隔符，只记录数据内容
                    line = "{name} {precision:.4f} {recall:.4f} {f1:.4f} {mcc:.4f}".format(
                        name=result['name'],
            ...
            print(f"Detailed results saved to {output_file_target}")
