...
# ========== Define constants
TASKS = ["RNAStrAlign", "bpRNA1m", "bpRNA_1m", "bpRNA_new", "Rivas","Filtered","crossfamily","Length_Short","Length_Medium"]
DATASETS = {
    "RNAStrAlign": ("RNAStrAlign.lst", "test.lst"),
    "bpRNA1m": ("TR0.lst", "TS0.lst"),
    "bpRNA_1m": ("train.lst", "test.lst"),
    "bpRNA_new": ("train.lst", "test.lst"),
    "Rivas": ("train.lst","test.lst"),
    "Filtered": ("Filtered_Train.lst","Filtered_Test.lst"),
    "crossfamily":("train.lst","test.lst"),
    "Length_Short":("train.lst","test_short.lst"),
    "Length_Medium":("train.lst","test_medium.lst")
...
parser.add_argument('--model_name_or_path',
                    type=str,
                    default="/home/cjj/RNAErnie/output/BERT,ERNIE,MOTIF,PROMPT/checkpoint_final/",
                    help='The build-in pretrained LM or the path to local model parameters.')
parser.add_argument('--model_path',
                    type=str,
                    default="/home/cjj/RNAErnie/output/BERT,ERNIE,MOTIF,PROMPT/checkpoint_final/model_state.pdparams",
                    help='The build-in pretrained LM or the path to local model parameters.')
...
# training args
parser.add_argument('--train', type=str2bool, default=False, help='Whether train the model.')
parser.add_argument('--epochs', type=int, default=50, metavar='N', help='number of epochs to train (default: 10)')
parser.add_argument('--disable_tqdm', type=str2bool, default=True, help='Disable tqdm display if true.')
...
parser.add_argument('--metrics',
                    type=str2list,
                    default="F1s,Accuracy,Precision,Recall,mcc",
...
if __name__ == "__main__":
    ...
    print("可见GPU数量:", paddle.device.cuda.device_count())
    print("当前设备:", paddle.device.get_device())
    print('Verson',paddle.version.cuda())
    import ctypes
    print('ctype:',ctypes.CDLL('libcudart.so').cudaRuntimeGetVersion())
    ...
    print(f'dataset path: {args.dataset_dir}/{args.dataset_train}')
    train_dataset = BPseqDataset(args.dataset_dir, args.dataset_train)
    # 打印數據集基本信息
    print(f"Train dataset loaded: {train_dataset}")
