...
class Exp:
    ...
    #     print_log('Test F1: {0:.4f}, Precision: {1:.4f}, Recall: {2:.4f}, MCC: {2:.4f}, Runtime: {3:.4f}\n'.format(test_f1, test_precision, test_recall, test_mcc, test_runtime))
    ...
    def test(self):
        ...
        test_f1, test_precision, test_recall, test_mcc, test_runtime = self.method.test_one_epoch_record(self.test_loader)
    ...
    def p_test(self):
        ...
        target_metrics,other_metrics = self.method.pes_test(self.test_loader)
    ...
    #     best_f1 = 0
    #     for epoch in range(self.args.epoch):
    #         train_loss,train_f1, train_precision, train_recall, train_mcc, train_time = self.method.train_one_epoch(self.train_loader)
    #         val_f1, val_precision, val_recall, val_mcc, _ = self.method.test_one_epoch(self.val_loader)
    #
    #         # 打印日志
    #         print_log(f'Epoch {epoch + 1}/{self.args.epoch}')
    #         print_log(f'Train Loss: {train_loss:.4f}')
    #         print_log(f'Val F1: {val_f1:.4f} Precision: {val_precision:.4f} Recall: {val_recall:.4f} MCC: {val_mcc:.4f}')
    #
    #         # 保存最佳模型
    #         if val_f1 > best_f1:
    #             best_f1 = val_f1
