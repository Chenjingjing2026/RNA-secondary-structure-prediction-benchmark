...
def model_eval_all_test(contact_net, test_generator):
    ...
    # 在循环开始前初始化
    detailed_results = []  # 用于存储NT指标的详细结果
    nc_detailed_results = []  # 用于存储NC指标的详细结果
    ...
    for contacts, seq_embeddings, matrix_reps, seq_lens, seq_ori, seq_name, nc_map, l_len in test_generator:
        ...
        # # 将 map_no_train 写入txt文件
        # with open('/home/chenjingjing/Models/UFold/UFold/data/pes/test.txt', 'w') as f:
        #     for i in range(map_no_train.shape[0]):
        #         # 将每个样本的矩阵转换为字符串格式写入
        #         matrix_str = '\n'.join([' '.join(map(str, row)) for row in map_no_train.cpu()[i].numpy()])
        #         f.write(f"Sample {i}: {seq_name}\n")
        #         f.write(matrix_str)
        #         f.write("\n" + "=" * 50 + "\n")  # 分隔线

        result_no_train += result_no_train_tmp

        # 记录每个样本的指标到txt文件 - 表格格式
        for i, (p_val, r_val, f1_val, mcc_val) in enumerate(result_no_train_tmp):
            sample_name = seq_name[i] if isinstance(seq_name, list) else seq_name
        ...
        if nc_map_nc.sum() != 0:
            ...
            # # 记录NC指标的每个样本
            # for i, (p_val, r_val, f1_val, mcc_val) in enumerate(result_nc_tmp):
            #     sample_name = seq_name[i] if isinstance(seq_name, list) else seq_name
            #     nc_detailed_results.append(f"{sample_name} {p_val:.4f} {r_val:.4f} {f1_val:.4f} {mcc_val:.4f}")
    ...
    ct_file.close()
    '''
    # 写入NT (non-training) 指标的详细结果
    output_file_nt = '/home/chenjingjing/Models/UFold/UFold/models/length/record/medium_metrics.txt'
    os.makedirs(os.path.dirname(output_file_nt), exist_ok=True)
    with open(output_file_nt, 'w') as f:
        for result in detailed_results:
            f.write(result + '\n')

    # # 如果有NC指标，也写入文件
    # if len(nc_detailed_results) > 0:
    #     output_file_nc = '/home/chenjingjing/Models/UFold/UFold/results/nc_test_metrics.txt'
    #     os.makedirs(os.path.dirname(output_file_nc), exist_ok=True)
    #     with open(output_file_nc, 'w') as f:
    #         for result in nc_detailed_results:
    #             f.write(result + '\n')
    #
...
def main():
    ...
    if test_file in ['TS1', 'TS2', 'TS3']:
        ...
        print('33')
    elif test_file == 'bprna_1m' or test_file == 'bprna_new' or test_file == 'bprna_pes' or test_file == 'rivas_test':
        ...
        MODEL_SAVED = f'/home/chenjingjing/Models/UFold/UFold/models/crossfamily_short/{test_file}/models/ufold_train_99.pt'
        ...
        print('66')
    elif test_file == 'pes' or test_file == 'pes_bprnanew' or test_file == 'pes_bprna1m':
    ...
    elif test_file == 'rivas_test':
        print('9')
        test_data = (RNASSDataGenerator('/home/chenjingjing/Models/UFold/UFold/data/Rivas/', 'TestSetB.cPickle'))
    elif test_file == 'bprna_1m' or test_file == 'pes_bprna1m':
        print('3')
        test_data = RNASSDataGenerator('/home/chenjingjing/Models/UFold/UFold/data/bprna_1m/', 'test.pickle')
    elif test_file == 'bprna_new' or test_file == 'pes_bprnanew':
        print('4')
        test_data = RNASSDataGenerator('/home/chenjingjing/Models/E2Efold/data/bprna_new/', 'bprna_new.pickle')
    elif test_file in ['16s','5s','23s','grp1','grp2','RNaseP','srp','telomerase','tmRNA','tRNA']:
        print('5')
        test_data = RNASSDataGenerator(f'/home/chenjingjing/Models/UFold/UFold/data/crossfamily_short/{test_file}/', 'test.pickle')
