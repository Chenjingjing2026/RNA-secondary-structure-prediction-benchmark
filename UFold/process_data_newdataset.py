...
# file_dir = '/home/chenjingjing/DATA/data/Crossfamily/tRNA/test'
#
# path= '/home/chenjingjing/Models/RFold/RFold-master/mydata/crossfamily_short/tRNA'
# save_path = path+'/test.pickle'
# length=1005

#
path='/home/chenjingjing/Models/UFold/UFold/data/crossfamily_short/tRNA'
save_path = path+"/test.pickle"
length=512
...
if __name__=='__main__':
    ...
    for index,item_file in enumerate(all_files):
        ...
        # file_lines = t0[1].split('\n')
        # # 跳过开头的注释行（以 # 开头的行），并且去掉空行
        # # 只跳过以 # 开头的注释行
        # filtered_lines = []
        # for line in file_lines:
        #     if line.startswith('#'):
        #         continue  # 跳过 # 开头的行（注释行）
        #     elif line.strip():  # 保证不处理空行
        #         filtered_lines.append(line)
        # filtered_lines = [line for line in file_lines if not line.startswith('#') and line.strip() != '']
        # seq = ''.join(filtered_lines)
        #pdb_file.write('>%s\n%s\n'%(item_file,seq.upper()))
        ...
        if t0[0] == 0:
            try:
                ...
                pdb.set_trace()
            # 检查序列是否包含非AUGC字符
            # if all(base in 'AUCG' for base in seq.upper()):
            #     one_hot_matrix = one_hot(seq.upper())
            #     # try:
            #     #     one_hot_matrix = one_hot(seq.upper())
            #     # except Exception as e:
            #     #     pdb.set_trace()  # 如果发生错误，进行调试
            # else:
            #     print(f"跳过无效序列：{seq}")
            #     continue  # 跳过无效序列，继续处理下一个文件
        ...
        if seq_len > 0 and seq_len <= length:
    ...
    ##      cut all to 600 length
            #pdb.set_trace()
            #print index
            one_hot_matrix_600 = np.zeros((length,4))
            # print(one_hot_matrix)

            one_hot_matrix_600[:seq_len,] = one_hot_matrix
            ss_label_600 = np.zeros((length,3),dtype=int)
    ...
    #     if seq_len > 0:  # 移除了<=1005的长度限制
    #         ss_label = np.zeros((seq_len, 3), dtype=int)
    #         print('item_file', item_file)
    #         ss_label[[*pair_dict_all.keys()],] = [0, 1, 0]  # 设置键位置的标签
    #         ss_label[[*pair_dict_all.values()],] = [0, 0, 1]  # 设置值位置的标签
    #         ss_label[np.where(np.sum(ss_label, axis=1) <= 0)[0],] = [1, 0, 0]  # 设置未配对位置的标签
    #
    #         # 直接使用原始长度，不进行填充
    #         sample_tmp = RNA_SS_data(
    #             seq=one_hot_matrix,  # 使用原始one_hot_matrix
    #             ss_label=ss_label,  # 使用原始ss_label
    #             length=seq_len,
    #             name=seq_name,
    ...
    cPickle.dump(all_files_list,open(save_path,"wb"))
