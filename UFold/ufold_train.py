...
cudnn.enabled = True # 确保 cuDNN 被启用
def train(contact_net,train_merge_generator,epoches_first):
    ...
    epoch_rec = []
    for epoch in tqdm(range(epoches_first), desc="Training Epochs"):
        contact_net.train()
        ...
        #for contacts, seq_embeddings, seq_embeddings_1, matrix_reps, seq_lens, seq_ori, seq_name in train_generator:
            ...
            # Compute loss
            # print('pred_contacts', pred_contacts.shape)
            # print('contacts_batch', contacts_batch.shape)  # 修正这里
            # print('contact_masks', contact_masks.shape)
            ...
            if steps_done % 10 == 0:  # 每10个batch打印一次内存情况
        ...
        if epoch > -1:
            ...
            torch.save(contact_net.state_dict(),  f'/home/chenjingjing/Models/UFold/UFold/models/crossfamily_short/tRNA/models/ufold_train_{epoch}.pt')
...
def main():
    ...
    for file_item in train_files:
        ...
        if file_item == 'RNAStralign' or file_item == 'ArchiveII':
            train_data_list.append(RNASSDataGenerator('/home/chenjingjing/Models/UFold/UFold/data/ArchiveII/', file_item + '.pickle'))
            print('Loading dataset:', file_item)
        elif file_item == 'Rivas_A' or file_item == 'TestSetB':
            train_data_list.append(RNASSDataGenerator('/home/chenjingjing/Models/UFold/UFold/data/Rivas/', file_item + '.cPickle'))
        elif file_item == 'bprna_1m':
            train_data_list.append(RNASSDataGenerator('/home/chenjingjing/Models/UFold/UFold/data/bprna_1m/', 'train.pickle'))
        elif file_item in ['16s','5s','23s','grp1','grp2','RNaseP','srp','telomerase','tmRNA','tRNA']:
            train_data_list.append(
                RNASSDataGenerator(f'/home/chenjingjing/Models/UFold/UFold/data/crossfamily_short/{file_item}/', 'train.pickle'))
        elif file_item in ['short','medium']:
            train_data_list.append(
                RNASSDataGenerator(f'/home/chenjingjing/Models/UFold/UFold/data/length/', 'train.pickle'))
        elif file_item =='pes':
            train_data_list.append(
                RNASSDataGenerator(f'/home/chenjingjing/Models/UFold/UFold/data/pes/', 'train.pickle'))
        else:
    ...
    #     contact_net = nn.DataParallel(contact_net)

    contact_net = contact_net.to(device)
    # contact_net = nn.DataParallel(contact_net, device_ids=[3, 4])
    # contact_net=contact_net.to(device)
    # train_merge_generator=train_merge_generator.to(device)
    # epoches_first =epoches_first.to(device)
