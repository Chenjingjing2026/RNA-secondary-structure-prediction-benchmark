...
class RNASSDataGenerator(object):
    ...
    def upsampling_data(self):
        ...
        # print(name)  # 打印路径列表，检查路径格式
        d_type = np.array(list(map(lambda x: x.split('.')[0], name)))  # 按文件名前缀分割
        # d_type = np.array(list(map(lambda x: x.split('/')[2], name)))

        # d_type = np.array(list(map(lambda x: x.split('/')[2], name)))
        # print(type(self.data))  # 确保它是一个包含相同结构的列表
        # print([len(item) for item in self.data])  # 检查每个元素的长度
        # print(self.data)
        # 或者
        # for item in self.data:
        #     print(type(item), len(item) if hasattr(item, '__len__') else item)
