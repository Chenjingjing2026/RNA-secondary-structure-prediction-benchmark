...
dataset = 'RNAStrAlign'
#added
pretrain_modelpath = f'/home/cjj/LTP/model/512/pretrain_{dataset}.h5'
train_modelpath = f'/home/cjj/LTP/model/512/train_{dataset}.h5'
...
def pretrain(seq_list, label_atrix, batch_size, epochs):
    ...
    model.summary()
    # 把模型转到2块gpu上
    #model = multi_gpu_model(model, gpus = 2) #not use
    model.compile(optimizer=Adam(lr=2e-4, beta_1=0.95, beta_2=0.98, epsilon=1e-7),
...
def continue_train(model_name, seq_list, label_atrix, batch_size, epochs):
    ...
    M = constraint_matrix_batch(seq_list)
    td = TensorBoard(log_dir='../logs/bprna_1m_model', histogram_freq=0, write_graph=True, write_images=True)
    rp = keras.callbacks.ReduceLROnPlateau(
        monitor='val_loss',
        factor=0.7,
        patience=3,
...
def start_predict(model_name, start_rate, end_rate, seq_list, label_atrix, batch_size):
    ...
    for i in range(len(seq_list)):
        single_seq = seq_list[i:i+1]  # shape: (1, seq_length)
        single_label = label_atrix[i:i+1]  # shape: (1, seq_length, seq_length)
        # 為單條序列生成約束矩陣
        single_M = constraint_matrix_batch(single_seq)  # shape: (1, seq_length, seq_length)
        
        # 預測單條
        single_prediction = model.predict((single_seq, single_M), batch_size=1, verbose=0)
        
        # 計算評估指標
        P, R, f1, cell = val_test(single_label, single_prediction)
        ...
        # 計算 F1 loss
        F1_loss_value = (-1) * F1_loss(single_label, single_prediction)
        
        precision_value = P.numpy() if hasattr(P, 'numpy') else float(P)
        recall_value = R.numpy() if hasattr(R, 'numpy') else float(R)
        f1_value = f1.numpy() if hasattr(f1, 'numpy') else float(f1)
        mcc_value = MCC.numpy() if hasattr(MCC, 'numpy') else float(MCC)
        # 儲存到 lists
        precision_list.append(precision_value)
        recall_list.append(recall_value)
        f1_list.append(f1_value)
        f1_loss_list.append(F1_loss_value.numpy())
        mcc_list.append(mcc_value)
        tp_list.append(cell[0].numpy())
        fp_list.append(cell[1].numpy())
        tn_list.append(cell[2].numpy())
        fn_list.append(cell[3].numpy())
        
        # 進度顯示（每10條顯示一次）
        if (i + 1) % 10 == 0:
            print(f'已處理 {i+1}/{len(seq_list)} 條序列')
        detailed_results.append(f"{i} {precision_value:.4f} {recall_value:.4f} {f1_value:.4f} {mcc_value:.4f}")
        # 写入详细结果到txt文件
        output_file = f'/home/cjj/LTP/results/{dataset}_test_metrics.txt'
...
Save_dir = f'/home/cjj/LTP/data/{dataset}/'
traindata = np.load(Save_dir+'train.npz')
testdata = np.load(Save_dir+'test.npz')

seq_list_train = traindata['arr_0']
label_atrix_train = traindata['arr_1']
# SHAPE_LABEL = data['arr_2']
seq_list_test = testdata['arr_0']
label_atrix_test = testdata['arr_1']
batch_size = 100
epochs = 150

#for test
# seq_list_train = seq_list_train[0:100]
# label_atrix_train = label_atrix_train[0:100]
# print('len seq_list', len(seq_list_train))
# print('len label_atrix', len(label_atrix_train))
# seq_list_test = seq_list_test[0:100]
# label_atrix_test = label_atrix_test[0:100]
# print('len seq_list', len(seq_list_test))
# print('len label_atrix', len(label_atrix_test))

Train = False
if Train:
    pretrain(seq_list_train[0:int(0.5 * len(seq_list_train))], 
             label_atrix_train[0:int(0.5 * len(seq_list_train))], 
             batch_size, 
             epochs)

    # 訓練集的後 50%
    start_train(seq_list_train[int(0.5 * len(seq_list_train)):], 
                label_atrix_train[int(0.5 * len(seq_list_train)):], 
                batch_size, 
                epochs)

# continue_pretrain(seq_list, label_atrix, batch_size, epochs)
# start_train(seq_list[0:int(0.9*len(seq_list_train))], label_atrix[0:int(0.9*len(seq_list_train))], batch_size, epochs)
...
start_predict(train_modelpath, 0.0, 1.0, seq_list_test, label_atrix_test, batch_size)
