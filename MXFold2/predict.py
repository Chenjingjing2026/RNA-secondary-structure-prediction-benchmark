...
class Predict:
    ...
    def predict(self, output_bpseq=None, output_bpp=None, result='/home/chenjingjing/Models/MXfold2/mxfold2-0.1.2/mxfold2/train.txt', use_constraint=False):
        res_fn = open(result, 'w') if result is not None else None
        self.model.eval()
        with torch.no_grad():
            for headers, seqs, refs in self.test_loader:
                print('headers, seqs, refs',headers, seqs, refs)
                start = time.time()
